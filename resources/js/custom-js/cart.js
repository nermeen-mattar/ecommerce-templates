...
function addToCart(e = {}) {
    ...
    if(product.type === "simple") {
        ...
        cart.products.push({id: product.id, quantity, recurring, name: product.name, price: product.price, image: product.images[0]?.src, index: cart.products.length});
    }
        ...
        cart.products.push({id: product.id, quantity, recurring, name: product.name, price: variation.price, image: variation.image?.src, index: cart.products.length});
        ...
        if(quantities.length) {
            quantities.each(function() {
                ...
                cart.products.push({id: groupedProduct.id, groupId: product.id, quantity, recurring, name: groupedProduct.name, price: groupedProduct.price, image: groupedProduct.images[0].src, index: cart.products.length})
              });
        } else {
            product.grouped_products.forEach(groupedProduct => {
                cart.products.push({id: groupedProduct.id, groupId: product.id, quantity: 1, recurring, name: groupedProduct.name, price: groupedProduct.price, image: groupedProduct.images[0].src, index: cart.products.length})
            });
        }
